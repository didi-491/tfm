...
Usamos el csv obtenido por DESeq2

```{r}
# 1. Leer los resultados de DESeq2 desde el archivo TSV
res <- read_tsv("Results/DEG_m6A_responder_vs_nonresponder.tsv")
# Convertir de SYMBOL a ENTREZID
gene_df <- bitr(res$Gene, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Hs.eg.db)

# Combinar la tabla original con los ENTREZID
res_annot <- merge(res, gene_df, by.x = "Gene", by.y = "SYMBOL")

# 2. Preparar la lista de genes ordenada por log2FoldChange
# Crear el vector nombrado
gene_list <- res_annot$log2FoldChange
names(gene_list) <- res_annot$ENTREZID

# Eliminar NAs y ordenar de mayor a menor
gene_list <- gene_list[!is.na(gene_list)]
gene_list <- sort(gene_list, decreasing = TRUE)

# 3. Realizar GSEA con Reactome pathways
gsea_result <- gseReactome(geneList = gene_list, 
                           organism = "human")      # Número máximo de genes p


# 4. Visualizar los resultados con un dotplot
dotplot(gsea_result)

# 5. Otras visualizaciones, como enrichMap
enrichMap(gsea_result)

```
El paquete anterior no funciona porque tengo muy pocos genes. vamos a probar con Enrich

```{r}
# Definir la lista de genes de la maquinaria m6A (asegúrate de que sean símbolos de genes)
# Lista de genes m6A: m6a_genes$Gene_symbol
epitranscriptomic_genes <- fread("D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/epitranscriptomic_regulators_annotated.txt", 
                                 sep = "\t", header = TRUE)
genes_m6A <- epitranscriptomic_genes %>%
  filter(Mark == "m6A") %>%
  pull(Gene_symbol)


# Realizar el análisis de enriquecimiento en varias bases de datos
enriched <- enrichr(genes_m6A, databases = c("GO_Biological_Process_2021", "KEGG_2021_Human", "Reactome_2022"))


# Guardar los resultados en archivos CSV
write.csv(enriched[["GO_Biological_Process_2021"]], "enriched_GO_Biological_Process.csv", row.names = FALSE)
write.csv(enriched[["KEGG_2021_Human"]], "enriched_KEGG_2021_Human.csv", row.names = FALSE)
write.csv(enriched[["Reactome_2022"]], "enriched_Reactome_2022.csv", row.names = FALSE)


# Ver los resultados de los diferentes enriquecimientos

```
```{r}
# Filtrar los términos de GO Biological Process para que solo se muestren los más significativos
# Por ejemplo, seleccionamos los términos con p-value < 0.05 o -log10(P-value) > 1.3 (~ p-value < 0.05)

filtered_GO <- enriched[["GO_Biological_Process_2021"]] %>%
  filter(P.value < 0.05)  # Puedes ajustar el umbral aquí

# Seleccionar un número limitado de los mejores términos
# Por ejemplo, seleccionamos los 20 términos más significativos
top_GO <- filtered_GO %>%
  arrange(P.value) %>%
  head(20)  # Ajusta el número de términos aquí según lo que necesites

# Visualización para GO Biological Process (Barplot) con ggplot2
p1 <- ggplot(top_GO, aes(x = -log10(P.value), y = Term, fill = -log10(P.value))) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(title = "Enrichment in GO Biological Process", x = "Enrichment Score (-log10(P-value))", y = "GO Term") +
  theme(axis.text.y = element_text(size = 6))

# Definir la ruta para guardar los gráficos
save_path <- "D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/Fotos/Enrichment/"

# Guardar el gráfico de GO Biological Process como un archivo PNG
ggsave(paste0(save_path, "Filtered_GO_Biological_Process_Enrichment.png"), plot = p1, width = 10, height = 6, dpi = 300)

# O guardar el gráfico como un archivo PDF
ggsave(paste0(save_path, "Filtered_GO_Biological_Process_Enrichment.pdf"), plot = p1, width = 10, height = 6)

# 2. Visualización para KEGG Pathways (Dotplot con ggplot2)
p2 <- ggplot(enriched[["KEGG_2021_Human"]], aes(x = -log10(P.value), y = Term, size = -log10(P.value), color = -log10(P.value))) +
  geom_point() +
  scale_color_viridis_c() +
  theme_minimal() +
  labs(title = "Enrichment in KEGG Pathways", x = "P-value (-log10)", y = "KEGG Pathway") +
  theme(axis.text.y = element_text(size = 6))

# Guardar el gráfico de KEGG Pathways como un archivo PNG
ggsave(paste0(save_path, "KEGG_Enrichment.png"), plot = p2, width = 10, height = 6, dpi = 300)

# O guardar el gráfico como un archivo PDF
ggsave(paste0(save_path, "KEGG_Enrichment.pdf"), plot = p2, width = 10, height = 6)

# 3. Visualización para Reactome (Dotplot con ggplot2)
p3 <- ggplot(enriched[["Reactome_2022"]], aes(x = -log10(P.value), y = Term, size = -log10(P.value), color = -log10(P.value))) +
  geom_point() +
  scale_color_viridis_c() +
  theme_minimal() +
  labs(title = "Enrichment in Reactome Pathways", x = "P-value (-log10)", y = "Reactome Pathway") +
  theme(axis.text.y = element_text(size = 6))

# Guardar el gráfico de Reactome Pathways como un archivo PNG
ggsave(paste0(save_path, "Reactome_Enrichment.png"), plot = p3, width = 10, height = 6, dpi = 300)

# O guardar el gráfico como un archivo PDF
ggsave(paste0(save_path, "Reactome_Enrichment.pdf"), plot = p3, width = 10, height = 6)
...
# 1. Leer datos
surv_response_data_pe <- read.table("D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/Protein_effect_annotation_KM.txt", header = TRUE, sep = "\t")
surv_response_data_pfs <- read.table("D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/Protein_effect_annotation_KM_PFS.txt", header = TRUE, sep = "\t")

# 2. Añadir Response_TAC1 y otras columnas
clinical_subset <- clinical_data %>% dplyr::select(sample_ID, response_TAC1)

# Merge con OS
data_pe <- merge(surv_response_data_pe, clinical_subset, by.x = "Sample", by.y = "sample_ID")

# Merge con PFS
data_pfs <- merge(surv_response_data_pfs, clinical_subset, by.x = "Sample", by.y = "sample_ID")

# 3. Eliminar Unknown
data_pe <- filter(data_pe, Mutational_Status != "Unknown")
data_pfs <- filter(data_pfs, Mutational_Status != "Unknown")

# Reemplazar comas por puntos y convertir a numérico
data_pe$OS_time_months <- as.numeric(gsub(",", ".", data_pe$OS_time_months))
data_pe$OS_status <- as.numeric(as.character(data_pe$OS_status))
data_pfs$PFS_time_months <- as.numeric(gsub(",", ".", data_pfs$PFS_time_months))
data_pfs$PFS_status <- as.numeric(as.character(data_pfs$PFS_status))

# 4. Modelo COX con todas las variables
cox_pe <- coxph(Surv(OS_time_months, OS_status) ~ Eraser + Writter + Reader + response_TAC1 + Mutational_Status, data = data_pe) # Meter clinicas 


cox_pfs <- coxph(Surv(PFS_time_months, PFS_status) ~ Eraser + Writter + Reader + response_TAC1 + Mutational_Status, data = data_pfs)

# 5. Extraer los resultados del modelo COX
results_pe <- tidy(cox_pe, exponentiate = TRUE, conf.int = TRUE)
results_pfs <- tidy(cox_pfs, exponentiate = TRUE, conf.int = TRUE)

# Redondear a 3 decimales
results_pe <- results_pe %>%
  mutate(across(c(estimate, conf.low, conf.high, p.value), round, 3))

results_pfs <- results_pfs %>%
  mutate(across(c(estimate, conf.low, conf.high, p.value), round, 3))

# 6. Preparar las etiquetas para ambos gráficos (OS y PFS)
results_pe$label <- paste0(results_pe$term, 
                           "\nHR=", results_pe$estimate, 
                           " [", results_pe$conf.low, "-", results_pe$conf.high, "]", 
                           "\nP=", results_pe$p.value)

results_pfs$label <- paste0(results_pfs$term, 
                            "\nHR=", results_pfs$estimate, 
                            " [", results_pfs$conf.low, "-", results_pfs$conf.high, "]", 
                            "\nP=", results_pfs$p.value)

# 7. Crear el gráfico de Forest plot para OS (Overall Survival)
png("D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/Fotos/Forest_Plot_OS.png", width = 800, height = 600)

# Usar ggforest de survminer

ggplot(results_pe, aes(y = reorder(term, estimate), x = estimate)) +
  geom_pointrange(aes(xmin = conf.low, xmax = conf.high), color = "blue") +
  geom_vline(xintercept = 1, linetype = "dashed", color = "red") +
  labs(title = "Forest Plot - OS", x = "Hazard Ratio", y = "") +
  theme_minimal() +
  geom_text(aes(label = label), hjust = -0.1, size = 3.5) +
  coord_cartesian(xlim = c(min(results_pe$conf.low) - 0.5, max(results_pe$conf.high) + 0.5)) +
  theme(axis.text.y = element_text(size = 10),
        plot.title = element_text(size = 14, face = "bold"),
        axis.text.x = element_text(size = 10)) 

# Cerrar el archivo PNG
dev.off()

# 8. Crear el gráfico de Forest plot para PFS (Progression-Free Survival)
png("D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/Fotos/Forest_Plot_PFS.png", width = 800, height = 600)

ggplot(results_pfs, aes(y = reorder(term, estimate), x = estimate)) +
  geom_pointrange(aes(xmin = conf.low, xmax = conf.high), color = "blue") +
  geom_vline(xintercept = 1, linetype = "dashed", color = "red") +
  labs(title = "Forest Plot - PFS", x = "Hazard Ratio", y = "") +
  theme_minimal() +
  geom_text(aes(label = label), hjust = -0.1, size = 3.5) +
  coord_cartesian(xlim = c(min(results_pfs$conf.low) - 0.5, max(results_pfs$conf.high) + 0.5)) +
  theme(axis.text.y = element_text(size = 10),
        plot.title = element_text(size = 14, face = "bold"),
        axis.text.x = element_text(size = 10)) # Ajuste de tamaño para la visibilidad
...
merged_os <- surv_response_data_pe %>%
  left_join(clinical_data %>%
              dplyr::select(Sample, sex, smoking_status, response_TAC1, age, stage),
            by = "Sample") %>%
  filter(!is.na(OS_time_months), !is.na(OS_status))
...
cox_model <- coxph(surv_obj ~ response_TAC1 + sex + smoking_status + age_group + stage, data = merged_os)

cox_summary <- summary(cox_model)

# ────────────────────────────────────────────────
# 4. Crear tabla para Forest Plot
# ────────────────────────────────────────────────
results <- data.frame(
  Variable = rownames(cox_summary$coefficients),
  HR = round(exp(cox_summary$coefficients[, "coef"]), 2),
  Lower_CI = round(exp(cox_summary$coefficients[, "coef"] - 1.96 * cox_summary$coefficients[, "se(coef)"]), 2),
  Upper_CI = round(exp(cox_summary$coefficients[, "coef"] + 1.96 * cox_summary$coefficients[, "se(coef)"]), 2),
  p_value = signif(cox_summary$coefficients[, "Pr(>|z|)"], 3)
)

# Eliminar NA o Inf que puedan romper el plot
results <- results %>% filter(is.finite(HR) & is.finite(Lower_CI) & is.finite(Upper_CI))

# Crear etiquetas combinadas
results <- results %>%
  mutate(label = paste0(Variable, "\nHR: ", HR, " [", Lower_CI, "-", Upper_CI, "]"))

# ────────────────────────────────────────────────
# 5. Visualización: Forest Plot
# ────────────────────────────────────────────────
# Filtrar filas con valores válidos para log-scale
results <- results %>%
  filter(HR > 0 & Lower_CI > 0 & Upper_CI > 0 & is.finite(HR) & is.finite(Lower_CI) & is.finite(Upper_CI))
...
# Forest plot
png("D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/Fotos/Forest_Cox_OS_Clinical.png", width = 1200, height = 900, res = 300)
forestplot(
  labeltext = results$label,
  mean = results$HR,
  ...
  title = "Cox Model for Overall Survival (Clinical Variables)",
...
# 8. CURVA ROC
# ───────────────────────────────────────────────────────────────
plot(roc_obj, col = "darkblue", lwd = 2, main = "ROC Curve - Random Forest (Todos los pacientes)")
legend("bottomright", legend = paste("AUC =", round(auc_val, 3)), col = "darkblue", lwd = 2)
...
metadata_model <- clinical_data %>%
  ...
  select(Sample, response_TAC1, sex, smoking_status)
...
model_data <- model_data %>%
  mutate(
    reader = ifelse(is.na(Reader), NA, as.numeric(Reader)),
    writer = ifelse(is.na(Writer), NA, as.numeric(Writer)),
    eraser = ifelse(is.na(Eraser), NA, as.numeric(Eraser))
  ) %>%
  select(-Reader, -Writer, -Eraser)
...
```
```{r}
# Expresión normalizada rlog_mat (código previo de DESeq2)

# Cargar clinical data
clinical_data_path <- "D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/20250228_DDBB_clinicos_RENAL_summarized.txt"
clinical_data <- fread(clinical_data_path, sep = "\t", header = TRUE)
...
# Cargar mutational annotations (ya binarizados Writer, Reader, Eraser)
mutation_types_by_patient <- read.table("D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/Mutation_type_annotation_binary.tsv", header = TRUE, sep = "\t")

# Asegurar columna Sample
mutation_types_by_patient$Sample <- rownames(mutation_types_by_patient)
...
metadata_model <- clinical_data %>%
  ...
  mutate(
    response_TAC1 = factor(response_TAC1, levels = c("non_responder", "responder")),
    sex = factor(sex, levels = c("male", "female")),
    smoking_status = na_if(as.character(smoking_status), "Unknown"),
    Sample = gsub("^htseq-RNA_FFPE_", "", sample_ID),
    Sample = gsub("\\.tsv$", "", Sample)
  ) %>%
  dplyr::select(Sample, response_TAC1, sex, smoking_status)
...
metadata_model <- metadata_model %>%
  left_join(mutation_types_by_patient %>% dplyr::select(Sample, Writer, Reader, Eraser), by = "Sample")
# Intersección de muestras
common_samples <- intersect(colnames(rlog_mat), metadata_model$Sample)

# Subset expresión
expr_data <- t(rlog_mat[, common_samples])
expr_data <- as.data.frame(expr_data)
expr_data$Sample <- rownames(expr_data)

# Unir todo
full_data <- metadata_model %>%
  filter(Sample %in% common_samples) %>%
  left_join(expr_data, by = "Sample") %>%
  drop_na()  # eliminar NA (smoking_status, mutaciones ausentes, etc.)
# Variables
X <- full_data %>%
  dplyr::select(-Sample, -response_TAC1)

# Convertir categóricas a dummies
X_model <- model.matrix(~ ., data = X)[, -1]  # elimina intercepto

# Respuesta binaria
y <- as.numeric(full_data$response_TAC1) - 1  # 0 = non_responder, 1 = responder

# Cross-validation LASSO
set.seed(123)
cv_lasso <- cv.glmnet(X_model, y, alpha = 1, family = "binomial")

# Mejor lambda
best_lambda <- cv_lasso$lambda.min

# Modelo final
lasso_model <- glmnet(X_model, y, alpha = 1, family = "binomial", lambda = best_lambda)

# Mostrar coeficientes no nulos
coef(lasso_model)[which(coef(lasso_model) != 0), ]
# Predicciones
pred_probs <- predict(lasso_model, newx = X_model, type = "response")
pred_labels <- ifelse(pred_probs > 0.5, 1, 0)

# Matriz de confusión
confusionMatrix(factor(pred_labels), factor(y), positive = "1")
