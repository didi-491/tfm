...
```{r}
# Tipos de mutaciones que afectan la proteína
protein_effect_mutations <- c("Missense_Mutation", 
                              "Nonsense_Mutation", 
                              "Frame_Shift_Del", 
                              "Frame_Shift_Ins", 
...
pathogenic <- m6A_maf@data %>%
  filter(
    Variant_Classification %in% protein_effect_mutations,
    grepl("deleterious", SIFT, ignore.case = TRUE),
    grepl("damaging", PolyPhen, ignore.case = TRUE),
    IMPACT != "LOW",
    gnomAD_AF < 0.01 | is.na(gnomAD_AF)
  )

write.table(pathogenic,
