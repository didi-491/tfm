...
```{r}
# Cargar el archivo TSV
maf_path <- "D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/m6A_filtered.maf"
clinical_data_path <- "D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/20250228_DDBB_clinicos_RENAL_summarized_maf.txt"
vc_nonsyn <- c("Targeted_Region", "Splice_Site", "Nonsense_Mutation", 
               "Frame_Shift_Del", "Frame_Shift_Ins", "Nonstop_Mutation", 
               "Translation_Start_Site", "In_Frame_Ins", "In_Frame_Del", 
...
# Convertir el archivo TSV en un objeto MAF sin guardarlo
m6A_oncoplot <- read.maf(maf = maf_path, clinicalData = clinical_data, vc_nonSyn = vc_nonsyn)

# Crear histograma
ggplot(m6A_oncoplot@data, aes(x = t_AF)) +
  geom_histogram(binwidth = 0.05, fill = "coral1", color = "black", alpha = 0.7) +
  labs(title = "Distribución de Frecuencias Alélicas (t_AF)",
       x = "Frecuencia Alélica (t_AF)",
       y = "Número de Variantes") +
...
m6A_oncoplot <- subsetMaf(maf=m6A_maf, query="t_depth > 10 & t_alt_count > 3")
...
```{r}
# Cargar el archivo TSV
maf_path <- "D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/merged_data.maf"
clinical_data_path <- "D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/20250228_DDBB_clinicos_RENAL_summarized.txt"
vc_nonsyn <- c("Targeted_Region", "Splice_Site", "Nonsense_Mutation", 
               "Frame_Shift_Del", "Frame_Shift_Ins", "Nonstop_Mutation", 
               "Translation_Start_Site", "In_Frame_Ins", "In_Frame_Del", 
...
m6A_tAF_check_intron <- subsetMaf(maf=merged_data, query="t_depth > 10 & t_alt_count > 3 & Variant_Classification != 'Intron'")

# Crear histograma
ggplot(m6A_tAF_check_intron@data, aes(x = t_AF)) +
  geom_histogram(binwidth = 0.05, fill = "coral1", color = "black", alpha = 0.7) +
  labs(title = "Distribución de Frecuencias Alélicas (t_AF)",
       x = "Frecuencia Alélica (t_AF)",
       y = "Número de Variantes") +
...
```{r}
m6A_tAF_check <- subsetMaf(maf=merged_data, query="t_depth > 10 & t_alt_count > 3 & Variant_Classification != 'Intron'")

png("D:/didic/Documents/Documentos/Master UOC/Cuatri 2/TFM/Fotos/t_AF_filtered.png", width = 2000, height = 1000, res = 300)

# Crear histograma
ggplot(m6A_tAF_check@data, aes(x = t_AF)) +
  geom_histogram(binwidth = 0.05, fill = "coral1", color = "black", alpha = 0.7) +
  labs(title = "Distribución de Frecuencias Alélicas (t_AF)",
       x = "Frecuencia Alélica (t_AF)",
       y = "Número de Variantes") +
